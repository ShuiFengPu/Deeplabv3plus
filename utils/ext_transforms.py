...
class ExtCenterCrop(object):
    ...
    Args:
        size (sequence or int): Desired results size of the crop. If size is an
...
class ExtRandomRotation(object):
    ...
    Args:
        ...
        expand (bool, optional): Optional expansion flag.
            If true, expands the results to make it large enough to hold the entire rotated image.
            If false or omitted, make the results image the same size as the input image.
...
class ExtRandomCrop(object):
    ...
    def get_params(img, output_size):
        ...
        Args:
            img (PIL Image): Image to be cropped.
            output_size (tuple): Expected results size of the crop.
        Returns:
...
class ExtResize(object):
    ...
    Args:
        size (sequence or int): Desired results size. If size is a sequence like
            (h, w), results size will be matched to this. If size is an int,
    ...
    def __repr__(self):
        interpolate_str = self._interpolation_str_map.get(
            self.interpolation,
            f"unknown({self.interpolation})"
        )
        return self.__class__.__name__ + '(size={0}, interpolation={1})'.format(self.size, interpolate_str) 
